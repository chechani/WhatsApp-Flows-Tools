...
// 		  params: {
// 			flow_token: "flows-builder-4e43c23b",
...
const SCREEN_RESPONSES = {
  ...
  export const getNextScreen = async (decryptedBody) => {
	...
	if (action === "data_exchange" && screen === "APPOINTMENT") {
	  try {
		// Assuming an API endpoint that returns available times based on department and location
		...
		// Use the response from the API to build the screen data
		const availableTimes = responseData; // Assuming the API returns a list of times
		...
		return {
		  ...
		  data: {
			...
			time: availableTimes.map(time => ({
			  ...
			  title: time.department,
			  enabled: true, // Assuming all returned times are available
			})),
